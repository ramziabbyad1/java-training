...
//make three copeis of cube, three of cylinder,three of mat translate to different positions, when setting color choose minimum
public class RayTracer extends MISApplet{
    ...
    public boolean computeColor(int rgb[], double v[], double w[]){	
		...
		Ray ray1 = new Ray(v, w);//v and w are computed from inverseViewport(x, y)
		...
		if (size1 == 0 && size2 ==0 && size3 == 0 ) {//if don't hit anything return the color of light
			for (int i = 0; i < 3; i++){
			...
			for (int i = 0; i < 3; i++){
				...
				//System.out.println(Arrays.toString(surfaceNormal));
			...
			if (!computeColor(rgbReflected, vReflected, wReflected)){//if the reflection doesn't hit anything, fully reflect the phong color
				for (int i = 0; i < 3; i++){
    ...
    public void setPixel(int x, int y, int rgb[]) { // SET ONE PIXEL'S COLOR
		inverseViewport(x, y);
		if(!computeColor(rgb, focalPoint, direction)){
			for (int i = 0; i < 3; i++){
				rgb[i] = 100;
			}
		}	
