...
public class Assignment10 extends MISApplet{
            ...
            if (shape.vertices != null && shape.faces != null) {
                ...
                for (int i = 0 ; i < shape.faces.length ; i++) {
                    ...
                    for (int j = 0 ; j < 4 ; j++) {
                        ...
                        double[] n =shape.getNormal(surfacePoint,shape.shapeID);
    ...
    public double[][] transpose(double[][] v){
    	...
    	double[][] temp = new double[v[0].length][v.length];
    	for(int i = 0; i < v[0].length;i++) 
    		for(int j=0; j < v.length;j++) temp[i][j] = v[j][i];
